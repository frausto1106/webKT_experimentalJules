    ...
    pricing: function () {
        ...
        if (e.isVariableDefined(p)) {
          ...
          pWrap.forEach(item => {
            ...
            // debugging info
            try {
              console.debug('pricing: found price-wrap', item, 'switch?', !!priceSwitch, 'prices count', priceElement ? priceElement.length : 0);
            } catch (err) {}

            // helper to apply prices based on toggle state
            function applyPrices() {
              try {
                var checked = priceSwitch ? !!priceSwitch.checked : false;
                console.debug('pricing.applyPrices called. switch checked=', checked);
                if (!priceElement) return;
                if (checked) {
                  priceElement.forEach(pItem => {
                    var dd = pItem.getAttribute('data-annual-price');
                    if (dd) pItem.innerHTML = dd;
                  });
                } else {
                  priceElement.forEach(pItem => {
                    var ee = pItem.getAttribute('data-monthly-price');
                    if (ee) pItem.innerHTML = ee;
                  });
                }
              } catch (err) {
                console.warn('pricing.applyPrices error', err);
              }
            }

            // initialize immediately (so page shows correct price without waiting for change)
            applyPrices();

            // bind change/click event if switch exists
            if (priceSwitch) {
              priceSwitch.addEventListener('change', applyPrices);
              priceSwitch.addEventListener('click', applyPrices); // fallback
            }
          });
        }

        // Global fallback: if .price-wrap container doesn't enclose the switch and prices,
        // bind directly to any .price-toggle and all .plan-price on the page.
        try {
          var globalSwitch = document.querySelector('.price-toggle');
          var globalPrices = document.querySelectorAll('.plan-price');
          if (globalPrices && globalPrices.length) {
            function applyPricesGlobal() {
              try {
                var checked = globalSwitch ? !!globalSwitch.checked : false;
                console.debug('pricing.applyPricesGlobal called. switch checked=', checked);
                globalPrices.forEach(function(pItem) {
                  var val = checked ? pItem.getAttribute('data-annual-price') : pItem.getAttribute('data-monthly-price');
                  if (val) pItem.innerHTML = val;
                });
              } catch (err) { console.warn('applyPricesGlobal error', err); }
            }
            // initialize
            applyPricesGlobal();
            if (globalSwitch) {
              globalSwitch.addEventListener('change', applyPricesGlobal);
              globalSwitch.addEventListener('click', applyPricesGlobal);
            }
          }
        } catch (err) { console.warn('pricing global fallback error', err); }
    },
    // END: Pricing
